/* =============================================================================
   ...
   - Arduino_FreeRTOS.h (RTOS library)
...
// RTOS tasks
void TaskServoService(void* pvParameters);
...
void setup() {
   ...
   // CRITICAL: Minimal stack sizes for 2KB RAM
   xTaskCreate(TaskServoService, "Servo", 96, NULL, 0,
               NULL);                                  // Higher priority
   xTaskCreate(TaskMove, "Move", 512, NULL, 1, NULL);  // Lower priority
...
/**
 * @brief main loop
 * @warning empty because of  RTOS
 */
void loop() {}

/**
 * @brief servo service task running at 50Hz (every 20ms)
 */
void TaskServoService(void* pvParameters) {
   (void)pvParameters;
...
/**
 * @brief main movement loop of the spiderbot
 */
void TaskMove(void* pvParameters) {
   (void)pvParameters;

   vTaskDelay(pdMS_TO_TICKS(1000));  // Wait for servo service to start
